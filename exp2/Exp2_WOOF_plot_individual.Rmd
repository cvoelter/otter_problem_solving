...
agg.data<-all.data%>%
  ...
  summarise(latency=mean(Latency), success=mean(Success))
...
agg.data.turn<-all.data%>%
  filter(Number_of_turns>0)%>%
  group_by(Subject, Phase, Condition, con2, food_level)%>%
  summarise(turns=mean(Number_of_turns), min.turns=min(Number_of_turns), max.turns=max(Number_of_turns))
...
turns.plot<- ggplot(data=agg.data.turn, aes(x=as.factor(Phase), y=turns))+
  ...
  geom_point()+
 # geom_errorbar(aes( x=Phase, ymin = min.turns, ymax = max.turns ))+
  geom_point(data=turn.data, aes( x=Phase, y=Number_of_turns ), pch=1)+
  labs(
    y = "Number of turns",
    x = "")+
  geom_vline(xintercept = 3.5, lty=3)+
